...
void sendToUE(JointOrientation jointOrientations[], Joint joints[]);
...
void CBodyBasics::Update()
{
    ...
    JointOrientation jointRotationArray[JointType_Count] = {};
    Joint jointInfos[JointType_Count] = {};
    ...
    if (SUCCEEDED(hr))
    {
        ...
        for (int i = 0; i < _countof(ppBodies); ++i)
        {
            ...
            /*std::clog << isTracked<<std::endl;
            std::clog << "hello"<<std::endl;
            */
            //OutputDebugString((LPCWSTR)isTracked);
            if (isTracked) {
                ppBodies[i]->GetJointOrientations(_countof(jointRotationArray), jointRotationArray);
                ppBodies[i]->GetJoints(_countof(jointInfos), jointInfos);
                sendToUE(jointRotationArray, jointInfos);
            }
...
std::string write_Json(JointOrientation jointOrientations[], Joint joints[]) {
    ...
    std::ofstream os;
    os.open("E:\\Kinect\\BodyBasics-D2D\\demo.json", std::ios::app);
    os << JsonRoot;
    os.close();
...
void sendToUE(JointOrientation jointOrientations[], Joint joints[]) {
    ...
    socketAddr.sin_port = htons(8010);
    //int cRes = connect(clientSocket, (SOCKADDR*)&socketAddr, sizeof(SOCKADDR));
    const char* sendBuf = "test";
    std::string sendBufs = write_Json(jointOrientations, joints);
    sendBuf = sendBufs.c_str();
    
    int i = sendto(clientSocket,sendBuf,strlen(sendBuf)+1,0, (SOCKADDR*)&socketAddr,sizeof(socketAddr));
    /*std::ofstream os;
    os.open("E:\\Kinect\\BodyBasics-D2D\\demo.txt", std::ios::app);*/
    ...
    if (i < 0) {
        ...
        std::string temp = strerror(i);
        std::cout << i<< errno<<temp<<x;
    }
}
